...
frappe.require("/assets/js/data_import_tools.min.js", () => {
	erpnext.accounts.ImportPreviewDialog = class ImportPreviewDialog extends (
		frappe.data_import.ImportPreview
	) {
		constructor(opts) {
			// file extended to make import preview compatible in dialog without form
			super(opts);
		}
		...
		show_column_mapper() {
			let column_picker_fields = this.get_columns_for_picker(
				this.doctype
			);
			let changed = [];
			...
			let file_name = (this.frm.get_value("import_file") || "")
				.split("/")
				.pop();
			...
			return [].concat(
				...keys.map((key) => {
					return column_map[key].map((df) => {
						...
						if (doctype !== key) {
							let table_field = frappe.meta.get_docfield(
								doctype,
								key
							);
